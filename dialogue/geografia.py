"""
Lista oficial dos municipios de Santa Catarina, usada so pra decidir
se a cidade da pessoa fica fora do estado -- nesse caso, oportunidade
presencial do IFSC nunca e alcancavel, entao o alcance efetivo vira
"ead" (ver `dialogue/recommendation.py`).

Fonte: Secretaria de Estado da Assistencia Social, Trabalho e
Habitacao de SC -- lista de municipios por macrorregiao (295
municipios, a totalidade do estado).
"""
...
_MUNICIPIOS_SC = (
    ...
    "Calmon", "Camboriú", "Campo Alegre", "Campo Belo do Sul", "Campo Erê",
    ...
    "Lindóia do Sul", "Lontras", "Luiz Alves", "Luzerna", "Macieira", "Mafra",
    ...
    "Penha", "Peritiba", "Pescaria Brava", "Petrolândia", "Pinhalzinho",
    ...
    "Três Barras", "Treviso", "Treze de Maio", "Treze Tílias",
...
_MUNICIPIOS_SC_NORM = frozenset(_normaliza(m) for m in _MUNICIPIOS_SC)


def cidade_em_sc(cidade: str) -> bool:
    """Diz se `cidade` é um dos 295 municípios de Santa Catarina.

    Comparação insensível a caixa e acento. Cidade que não bate com
    nenhum município do estado é tratada como fora de SC -- usado pra
    forçar o alcance efetivo pra "ead" (oportunidade presencial do
    IFSC nunca alcança quem mora fora do estado).
    """
    return _normaliza(cidade) in _MUNICIPIOS_SC_NORM
